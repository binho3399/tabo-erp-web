    ...
    return (
        <div className={`group rounded-[20px] overflow-hidden transition-all duration-300 bg-white ${isOpen ? 'shadow-xl shadow-blue-500/5' : 'shadow-md hover:shadow-lg'}`}>
            <button
                ...
                onClick={onClick}
            >
                <span className={`text-[17px] font-medium leading-snug transition-colors duration-300 ${isOpen ? 'text-blue-600' : 'text-slate-800'}`}>
                    {question}
                </span>
            ...
            >
                <div className="px-6 pb-6 pt-1 text-[15px] text-slate-500 font-light leading-relaxed">
                    {answer}
                </div>
            </div>
        </div>
